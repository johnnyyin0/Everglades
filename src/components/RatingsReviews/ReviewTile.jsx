  ...
  return (
    <div className="border-b-2 border-l-2">
      ...
      <div className="px-5 pb-5">
        <span>{!showMore ? clippedReview + restReview : clippedReview}{showMore &&restReview && '...'}</span>
      <div>
        {showMore && <span className="float-right h-min hover:text-cyan-800 cursor-pointer" onClick={handleShowMore}>Show more</span>}
        {showLess && <span className="float-right h-min hover:text-cyan-800 cursor-pointer" onClick={handleShowLess}>Show less</span>}
      </div>
      </div>
